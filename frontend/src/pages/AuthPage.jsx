...
function AuthPage() {
  ...
  // ✅ AUTO-SUBMIT: Begitu 6 digit langsung verifikasi
  useEffect(() => {
    if (otp.length === 6) handleVerify2FALogin();
  }, [otp]);

  // 🔥 TRIGGER SEND OTP: Fungsi buat panggil pengiriman Email + WA
  const triggerSendOTP = async (userId) => {
    try {
      await api.post('/auth/send-otp', { userId });
      skuyAlert("SECURITY", "Kode OTP sudah dikirim ke Email & WhatsApp lo, Ri!", "info");
    } catch (err) {
      skuyAlert("ERROR", "Gagal mengirim OTP via Email/WA.", "error");
    }
  };
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      if (res.data.requiresTwoFA) {
        ...
        setShow2FA(true);
        // ✅ Langsung kirim kodenya pas login terdeteksi butuh 2FA
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isLogin && res.data.requiresTwoFA) {
        setTempUserId(res.data.userId);
        setShow2FA(true);
        // ✅ Langsung kirim kodenya ke Email/WA
        await triggerSendOTP(res.data.userId);
      } else if (res.data.success) {
        if (isLogin) {
          localStorage.setItem('user_token', res.data.token);
  ...
  const handleVerify2FALogin = async (e) => {
    ...
    try {
      ...
      skuyAlert("KODE SALAH", "OTP salah atau expired. Cek Email/WA lagi!", "error");
      setOtp('');
    } finally { setLoading(false); }
  };
          ...
          <div>
            ...
            <div className="space-y-5">
              <div className="flex items-center gap-3 text-slate-400 font-bold italic text-sm">
                <span className="text-[#7C3AED]"><MessageSquare size={18}/></span> Multi-Channel OTP (Email & WA)
              </div>
              <div className="flex items-center gap-3 text-slate-400 font-bold italic text-sm">
                <span className="text-[#7C3AED]"><ShieldCheck size={18}/></span> Encrypted User Vault
              </div>
            </div>
          </div>
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-600 italic">© 2026 SKUY.GG STUDIO</p>
        </div>
          ...
          <AnimatePresence mode="wait">
            {!show2FA ? (
              <motion.div key="form" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                ...
                {isLogin && (
                  <div className="mb-8">
                    <div className="border-4 border-slate-950 rounded-2xl overflow-hidden shadow-[4px_4px_0px_0px_#000]">
                ...
                <form className="space-y-4" onSubmit={handleSubmit}>
                  {!isLogin && (
                    <div className="relative group">
                      ...
                      <input type="text" placeholder="Username" required className="w-full bg-slate-50 border-2 border-slate-200 p-4 pl-14 rounded-xl font-bold focus:border-slate-950 outline-none transition-all" value={formData.username} onChange={(e) => setFormData({...formData, username: e.target.value})} />
                    </div>
                  )}
                  <div className="relative group text-left">
                    <Mail className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#7C3AED]" size={18}/>
                    <input type="email" placeholder="Email Address" required className="w-full bg-slate-50 border-2 border-slate-200 p-4 pl-14 rounded-xl font-bold focus:border-slate-950 outline-none transition-all" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} />
                  </div>
                  <div className="relative group">
                    <Lock className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#7C3AED]" size={18}/>
                    <input type="password" placeholder="Password" required className="w-full bg-slate-50 border-2 border-slate-200 p-4 pl-14 rounded-xl font-bold focus:border-slate-950 outline-none transition-all" value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} />
                  </div>
              ...
              <motion.div key="2fa" initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center space-y-8">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border-4 border-slate-950 p-6 rounded-2xl text-center text-6xl font-black tracking-[0.3em] outline-none shadow-inner focus:bg-white transition-all"
