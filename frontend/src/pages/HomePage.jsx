  ...
  useEffect(() => {
    const fetchStreamers = async () => {
      try {
        ...
        // Pastikan format response sesuai (Array atau Object.users)
    ...
    <div className={`min-h-screen ${darkMode ? 'bg-[#0A0A0C] text-white' : 'bg-[#F8FAFF] text-slate-900'} transition-colors duration-500 font-sans selection:bg-violet-600 selection:text-white overflow-x-hidden`}>
      ...
      <nav className="fixed top-0 w-full z-[100] p-6">
        <div className={`max-w-7xl mx-auto px-8 py-4 rounded-3xl border-4 border-slate-950 flex justify-between items-center backdrop-blur-md shadow-[6px_6px_0px_0px_#000] ${darkMode ? 'bg-white/5' : 'bg-white/80'}`}>
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/auth" className="hidden sm:block bg-slate-950 text-white text-[10px] font-black px-8 py-3 rounded-xl uppercase shadow-[4px_4px_0px_0px_#7C3AED] hover:translate-y-[-2px] transition-all">Dashboard</Link>
          </div>
        </div>
      </nav>
      ...
      <section className="relative pt-64 pb-32 px-6 text-center overflow-hidden">
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          <Link to="/auth" className="inline-block bg-violet-600 text-white font-black px-12 py-8 rounded-[2.5rem] border-4 border-slate-950 shadow-[10px_10px_0px_0px_#000] text-3xl uppercase italic hover:translate-y-[-4px] hover:shadow-none active:translate-y-2 transition-all">
            Join the Squad
          </Link>
        </motion.div>
      </section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {streamers.slice(0, 8).map((s) => (
              ...
              >
                ...
                <h4 className="text-xl font-black italic uppercase tracking-tighter mb-2 text-slate-950 dark:text-white">{s.full_name || s.username}</h4>
      ...
      <section className="max-w-4xl mx-auto px-6 py-40">
        ...
        <div className="space-y-6">
          {[
            ...
            <details key={i} className="group p-8 rounded-[2rem] border-4 border-slate-950 bg-white dark:bg-white/5 cursor-pointer shadow-[6px_6px_0px_0px_#000] open:shadow-none transition-all">
      ...
      {/* --- FOOTER (SLEEK & PROPORTIONAL) --- */}
      <footer className={`border-t-8 border-slate-950 py-16 ${darkMode ? 'bg-[#0A0A0C]' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-12">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-12">
            ...
            <Link to="/auth" className="bg-slate-950 text-white px-10 py-4 rounded-2xl font-black uppercase italic shadow-[6px_6px_0px_0px_#7C3AED] hover:translate-y-1 hover:shadow-none transition-all">GASS DASHBOARD</Link>
          </div>
        </div>
        <div className="max-w-7xl mx-auto px-6 mt-16 pt-8 border-t-2 border-slate-100 dark:border-white/5 text-[8px] font-black text-slate-400 uppercase tracking-[0.5em] text-center">
