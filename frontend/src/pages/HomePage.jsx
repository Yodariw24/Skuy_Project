...
function HomePage() {
  ...
  // --- 🛠️ PERBAIKAN LOGIKA FETCHING (ANTI WHITE SCREEN) ---
  useEffect(() => {
    const fetchStreamers = async () => {
      try {
        const res = await api.get('/streamers');
        // ✅ Perbaikan 1: Pastikan data selalu array (Mencegah n.map error)
        setStreamers(Array.isArray(res.data) ? res.data : []); 
      } catch (err) {
        console.warn("Koneksi Railway bermasalah, nampilin data fallback.");
        setStreamers([
          { id: 1, username: 'ariwirayuda', full_name: 'Ari Wirayuda', role: 'creator', bio: 'Informatics Engineering Student' }
        ]);
      }
    }
    fetchStreamers()
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-[#050505] text-white' : 'bg-[#FAFAFB] text-slate-900'} transition-colors duration-1000 font-sans selection:bg-violet-600 selection:text-white overflow-x-hidden relative`}>
          ...
          <motion.div whileHover={{ y: -10 }} className={`md:col-span-7 p-12 rounded-[4rem] border relative overflow-hidden group transition-all ${darkMode ? 'bg-white/5 border-white/5 shadow-[0_0_50px_rgba(0,0,0,0.3)]' : 'bg-white border-slate-100 shadow-2xl shadow-slate-200/40'}`}>
            ...
            <div className="flex justify-between items-start mb-16">
              <div className="space-y-4 w-full max-w-xs relative z-10">
                 {[{n: 'Sultan_Jakarta', a: '1.5M', m: 'Gacor parah bang!'}].map((d, i) => (
      ...
      {/* --- 🛠️ ELITE CREATORS (DENGAN PERBAIKAN FOTO & MAP) --- */}
      <section className="max-w-7xl mx-auto px-6 py-20 relative z-10 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 text-left">
          {/* ✅ Perbaikan 2: Fallback (streamers || []) agar tidak crash */}
          {(streamers || []).map((s) => (
            <motion.div key={s.id} whileHover={{ y: -15 }} className={`group p-10 rounded-[4rem] border transition-all ${darkMode ? 'bg-white/5 border-white/5 hover:bg-white/10' : 'bg-white border-slate-100 hover:shadow-2xl shadow-violet-900/10'}`}>
              <div className="w-20 h-20 bg-slate-100 dark:bg-white/10 rounded-[2.5rem] mb-10 overflow-hidden border-4 border-white dark:border-slate-800 shadow-xl group-hover:border-violet-600 transition-all">
                <img 
                  // ✅ Perbaikan 3: Jalur foto sinkron dengan uploads/ di Railway
              ...
              <p className="text-slate-400 text-[12px] font-bold mb-10 italic line-clamp-1">"{s.bio || 'Elite Creator SkuyGG'}"</p>
      ...
      <section className="max-w-4xl mx-auto px-6 py-24 relative z-10 text-left">
        <div className="flex items-center gap-6 mb-16">
          <div className="w-12 h-12 bg-violet-600 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-violet-600/30"><HelpCircle size={24}/></div>
          <h2 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter">FAQ <span className="text-violet-600">SQUAD</span></h2>
        </div>
        <div className="space-y-2">
          <FAQItem darkMode={darkMode} question="Apa itu Skuy.GG?" answer="Platform saweria ala Skuy untuk dukung kreator favorit lo tanpa potongan ribet." />
          <FAQItem darkMode={darkMode} question="Gimana cara dapet role Elite?" answer="Cukup daftar, aktif ngonten, dan lengkapi profil lo biar muncul di Homepage." />
        </div>
      </section>

      {/* --- FOOTER --- */}
      <footer className="mt-32 relative bg-violet-600 text-white overflow-hidden py-32">
        <div className="absolute top-1/2 left-0 -translate-y-1/2 flex whitespace-nowrap opacity-10 pointer-events-none">
          <motion.div animate={{ x: [0, -1000] }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }} className="text-[200px] font-black italic uppercase px-4">SKUY.GG SKUY.GG SKUY.GG</motion.div>
        </div>
        <div className="max-w-7xl mx-auto px-10 relative z-10 flex flex-col items-center text-center">
          <h2 className="text-7xl md:text-[130px] font-black italic uppercase leading-[0.75] mb-12 tracking-tighter">SIAP JADI<br/>LEGENDA?</h2>
          <Link to="/auth" className="bg-white text-violet-600 font-black px-16 py-7 rounded-[3rem] text-2xl uppercase italic inline-block transition-all hover:scale-105">Join Squad Kuy</Link>
          <div className="w-full mt-32 pt-10 border-t border-white/20 flex justify-between items-center text-[10px] font-black uppercase tracking-[0.5em]">
             <p>© 2026 Skuy.GG Studio • Karawang Pride</p>
          </div>
        </div>
      </footer>
