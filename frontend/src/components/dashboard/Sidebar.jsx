...
function Sidebar({ user }) {
  ...
  // ✅ SYNC LOGIC: Ambil ID menu dari URL (misal: /dashboard/tip -> 'tip')
  ...
  const NavButton = ({ id, icon: Icon, label, badge, disabled, onClickCustom, isSub }) => {
    // ✅ Check if active based on URL path
    ...
    return (
      <button 
        ...
        onClick={() => {
          ...
          else navigate(`/dashboard/${id}`); // ✅ Navigate to URL
        }}
  ...
  return (
    <aside className="w-64 bg-white border-r-4 border-slate-950 flex flex-col h-screen sticky top-0 overflow-hidden font-sans text-left shadow-[4px_0px_0px_0px_rgba(0,0,0,0.05)]">
      {/* HEADER LOGO */}
      <div className="p-8 flex items-center gap-3.5 group cursor-pointer" onClick={() => navigate('/')}>
