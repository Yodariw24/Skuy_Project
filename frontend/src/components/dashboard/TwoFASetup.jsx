...
import { ShieldCheck, Loader2, MessageSquare, Mail, CheckCircle2, AlertTriangle, ArrowRight } from 'lucide-react';
...
function TwoFASetup({ user }) {
  ...
  const handleRequestOTP = async () => {
    ...
    // 🛡️ PROTEKSI SULTAN: Redirect ke Profil kalau WA kosong
    if (!user?.phone_number || user?.phone_number.trim() === "") {
      return Swal.fire({
        ...
        if (result.isConfirmed) {
          // Asumsi lo pake navigasi atau tab di dashboard
          window.location.hash = "#profile"; // Atau sesuaikan dengan route lo
        }
      });
    }
    ...
    try {
      ...
      if (res.data.success) {
        setStep('verifying');
        skuyAlert("PROTOCOL SENT", "Cek WhatsApp & Email lo sekarang, Ri!", "info");
      }
    } catch (err) {
      skuyAlert("GAGAL", "Gagal kontak server OTP. Cek koneksi Railway lo!", "error");
    } finally {
      setLoadingOTP(false);
    }
  };
    ...
    <div className="bg-white p-8 rounded-[2rem] border-4 border-slate-950 shadow-[8px_8px_0px_0px_#7C3AED] text-left relative overflow-hidden">
      ...
      {step === 'enabled' && (
        <div className="flex flex-col items-center py-6 space-y-4">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-20 h-20 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center border-4 border-emerald-500 shadow-[4px_4px_0px_0px_#10B981]">
            <CheckCircle2 size={40} />
          </motion.div>
          <div className="text-center">
            <p className="font-black italic uppercase text-slate-950 text-lg">Status: PROTECTED</p>
            <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest italic">Akun lo aman terkendali lewat WA & Email</p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6">
          <div className="bg-slate-50 p-5 rounded-2xl border-4 border-slate-950 shadow-inner">
            <p className="text-[11px] font-bold text-slate-700 leading-relaxed italic">
              "Aktifkan Dual-OTP biar tiap lo login, SkuyGG kirim kode rahasia ke <span className="text-violet-600 underline">Email</span> dan <span className="text-emerald-600 underline">WhatsApp</span> lo."
            </p>
          </div>
          
          {!user?.phone_number && (
            <div className="flex items-center gap-2 p-3 bg-amber-50 border-2 border-amber-200 rounded-xl text-amber-700 text-[9px] font-black uppercase italic">
              <AlertTriangle size={14} /> Nomor WA belum diset! Isi di profil dulu, Ri.
            </div>
          )}
          ...
          >
            {loadingOTP ? <Loader2 className="animate-spin" /> : <>AKTIFKAN PROTOKOL <ArrowRight size={18}/></>}
          </button>
        </div>
      )}
      ...
      {step === 'verifying' && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <div className="flex justify-center gap-6 text-[#7C3AED]">
            <motion.div animate={{ y: [0, -5, 0] }} transition={{ repeat: Infinity, duration: 1.5 }}><Mail size={32} /></motion.div>
            <motion.div animate={{ y: [0, -5, 0] }} transition={{ repeat: Infinity, duration: 1.5, delay: 0.2 }}><MessageSquare size={32} /></motion.div>
          </div>

          <div className="space-y-3 text-center">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] italic">Masukkan 6 Digit Kode</p>
            <input 
          ...
          <div className="flex gap-4">
            <button onClick={() => { setStep('idle'); setOtp(''); }} className="flex-1 py-4 border-4 border-slate-950 rounded-2xl font-black uppercase text-[10px] hover:bg-slate-50 transition-all shadow-[4px_4px_0px_0px_#000] active:translate-y-1 active:shadow-none">Batal</button>
            <button onClick={handleActivate} disabled={isVerifying || otp.length < 6} className="flex-[2] py-4 bg-slate-950 text-white rounded-2xl font-black uppercase text-[10px] shadow-[4px_4px_0px_0px_#7C3AED] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center gap-2 disabled:bg-slate-300 disabled:shadow-none disabled:border-slate-400">
              {isVerifying ? <Loader2 className="animate-spin" /> : 'KONFIRMASI'}
            </button>
          </div>
        </motion.div>
      )}
