...
export default function ProfileSettings({ user, setUser }) {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await api.get('/donations/categories');
  ...
  // 📸 ACTION HANDLING 1: GANTI FOTO PROFIL
  ...
  // 🗑️ ACTION HANDLING 2: HAPUS FOTO PROFIL
  const handleDeletePhoto = async () => {
    const confirm = await Swal.fire({
      ...
      customClass: {
        ...
        confirmButton: 'bg-red-500 text-white text-[10px] font-black px-6 py-3 rounded-xl mx-2 uppercase italic border-2 border-slate-950 shadow-[3px_3px_0px_0px_#000]',
  ...
  // 📝 ACTION HANDLING 3: UPDATE DATA TEKS PROFIL
  ...
  const handleUpdateBank = async (bankDataFromModal) => {
    setLoadingText(true);
    try {
      const res = await api.put(`/user/bank/${user.id}`, bankDataFromModal);
      if (res.data.success) {
        // ✅ UPGRADE STRUCTURAL MAPPING: Mengunci struktur properti bank agar sinkron di seluruh views
        const updatedUser = { 
          ...user, 
          bank_name: bankDataFromModal.bank_name,
          bank_account_number: bankDataFromModal.bank_account_number,
          bank_account_name: bankDataFromModal.bank_account_name,
          account_number: bankDataFromModal.bank_account_number, // Fallback alias data lama
          account_name: bankDataFromModal.bank_account_name
        };
        setUser(updatedUser);
        localStorage.setItem('user', JSON.stringify(updatedUser));
  ...
  return (
    <div className="max-w-5xl mx-auto font-sans text-slate-900 pb-20 text-left selection:bg-violet-600 selection:text-white">
      ...
      {status.message && (
        <div className={`fixed top-10 right-10 z-[100] px-8 py-4 rounded-[2rem] shadow-2xl animate-in slide-in-from-right border-4 border-slate-950 ${status.type === 'success' ? 'bg-slate-900 text-violet-400' : 'bg-red-500 text-white'} font-black text-[10px] uppercase italic tracking-widest flex items-center gap-3`}>
          ...
          <p dangerouslySetInnerHTML={{ __html: status.message }}></p>
        </div>
      )}
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-violet-600 font-bold italic lowercase">{currentUrl}/{formData.username}</p>
            </div>
          </div>
          <button type="button" onClick={() => { navigator.clipboard.writeText(`${currentUrl}/${formData.username}`); setStatus({ type: 'success', message: 'Link Copied!' }); }} className="px-5 py-2.5 bg-slate-50 text-slate-600 rounded-xl font-black text-[10px] uppercase border border-slate-100 hover:bg-slate-100 transition-all flex items-center gap-2 cursor-pointer"><Icon.Copy size={14} /> Copy Link</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-8">
          <form onSubmit={handleUpdateTextData} className="bg-white p-8 md:p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
            ...
            <div className="w-full">
              <div className="flex items-center justify-between mb-2 px-1 text-left">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.15em] block">Kategori Konten Streaming 🎮</label>
              </div>
              <div className="relative group">
            ...
            {/* --- INTEGRASI ORNAMEN SOSIAL MEDIA SULTAN --- */}
            <div className="relative pt-2">
              <div className="w-full border-t-2 border-dashed border-slate-100 mb-6" />
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <FormInput label="Instagram URL" iconName="Instagram" placeholder="https://instagram.com/..." value={formData.instagram} onChange={(e) => setFormData({...formData, instagram: e.target.value})} />
                <FormInput label="TikTok URL" iconName="MessageCircle" placeholder="https://tiktok.com/@..." value={formData.tiktok} onChange={(e) => setFormData({...formData, tiktok: e.target.value})} />
                <FormInput label="YouTube URL" iconName="Youtube" placeholder="https://youtube.com/..." value={formData.youtube} onChange={(e) => setFormData({...formData, youtube: e.target.value})} />
              </div>
            </div>

            {/* TRIGGER TEKS PROFIL */}
            <button type="submit" disabled={loadingText} className="w-full py-4 bg-slate-950 text-white rounded-[1.5rem] font-black uppercase text-[11px] italic tracking-[0.2em] shadow-xl hover:bg-violet-600 transition-all flex items-center justify-center gap-3 border-0 cursor-pointer">
          ...
          <div className="bg-white p-8 md:p-10 rounded-[2.5rem] border-4 border-slate-950 shadow-[10px_10px_0px_0px_#f1f5f9] flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <button type="button" onClick={() => setIsBankModalOpen(true)} className="px-8 py-4 bg-white border-4 border-slate-950 text-slate-950 rounded-2xl font-black text-[10px] uppercase italic tracking-widest hover:bg-slate-950 hover:text-white transition-all active:translate-y-1 shadow-[4px_4px_0px_0px_#000] cursor-pointer">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col items-center">
            ...
            <div className="w-full flex flex-col gap-2.5">
              ...
              {formData.profile_picture && (
                <button type="button" disabled={loadingPhoto} onClick={handleDeletePhoto} className="w-full py-3 bg-red-50 text-red-600 rounded-xl font-black text-[10px] uppercase border border-red-100 hover:bg-red-600 hover:text-white hover:border-red-600 transition-all flex items-center justify-center gap-2 cursor-pointer">
      ...
      <EditBankModal 
        ...
        onSave={(data) => handleUpdateBank(data)} // Menerima data langsung dari form modal bank lo, Ri
        loading={loadingText} 
      />
    </div>
  )
