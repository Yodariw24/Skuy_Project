      ...
      <div className="overflow-x-auto rounded-xl border-2 border-slate-950">
        <table className="w-full text-left text-xs bg-white">
          ...
          <tbody className="divide-y-2 divide-slate-100 font-mono text-[11px]">
            {loading ? (
              <tr><td colSpan="7" className="p-6 text-center text-slate-400 font-black uppercase animate-pulse">Syncing midtrans cloud core blocks...</td></tr>
            ) : donations.length === 0 ? (
              <tr><td colSpan="7" className="p-6 text-center text-slate-400 font-black uppercase italic">Tidak ada sirkuit mutasi finansial, Ri!</td></tr>
            ) : (
              donations.map((tx) => (
                <tr key={tx.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-3">
                    <span className="px-2 py-1 bg-emerald-100 text-emerald-800 border-2 border-emerald-800 font-sans font-black text-[9px] rounded-md uppercase tracking-wider shadow-[1px_1px_0px_0px_#1e4620]">Settlement</span>
                  </td>
