import { ShieldCheck, Lock, Zap, Loader2, CheckCircle2, Mail, ShieldAlert } from 'lucide-react'
      ...
      <AnimatePresence mode="wait">
        {!isEnabled ? (
            ...
            {!otpSent ? (
              ...
              <div className="space-y-8">
                ...
                <div className="bg-amber-50 p-6 rounded-3xl border-2 border-dashed border-amber-300">
                  ...
                  <p className="text-[10px] text-amber-500 font-bold leading-tight">Masukkan 6 digit kode yang dikirim ke {user?.email}</p>
                </div>
          ...
          <div className="bg-white border-4 border-emerald-500 p-12 rounded-[3.5rem] text-center shadow-[10px_10px_0px_0px_#10b981]">
              ...
              <h3 className="text-2xl font-black italic uppercase text-emerald-600">Secure Active</h3>
              <p className="text-slate-400 text-xs font-bold mt-2 tracking-tight italic">Akun sultan aman terkendali.</p>
              <button onClick={onDisable} className="mt-8 text-[10px] font-black uppercase text-slate-300 hover:text-rose-500 transition-colors">Disable Security</button>
          </div>
        )}
      </AnimatePresence>
    </div>
